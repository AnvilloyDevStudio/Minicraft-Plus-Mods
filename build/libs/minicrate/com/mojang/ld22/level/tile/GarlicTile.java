...
public class GarlicTile extends Tile
{
    public GarlicTile(final int id) {
        super(id);
    }
    ...
    public void render(final Screen screen, final Level level, final int x, final int y) {
        ...
        if (icon >= 3) {
            ...
            if (age == 50) {
                col = Color.get(555, 0, 40 + icon * 100, 40 + (icon - 3) * 2 * 100);
            }
            icon = 3;
        }
    ...
    private void harvest(final Level level, final int x, final int y) {
        ...
        for (int count = this.random.nextInt(3), i = 0; i < count; ++i) {
            level.add(new ItemEntity(new ResourceItem(Resource.garlicSeeds), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
        }
        int count = 0;
        if (age == 50) {
        ...
        for (int i = 0; i < count; ++i) {
            level.add(new ItemEntity(new ResourceItem(Resource.garlic), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
        }
