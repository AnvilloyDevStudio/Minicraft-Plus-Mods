...
public class HardRockTile extends Tile
{
    ...
    public void render(final Screen screen, final Level level, final int x, final int y) {
        final int col = Color.get(334, 334, 223, 223);
        final int transitionColor = Color.get(1, 334, 445, level.dirtColor);
    ...
    public void hurt(final Level level, final int x, final int y, final int dmg) {
        ...
        if (damage >= 200) {
            for (int count = this.random.nextInt(4) + 1, i = 0; i < count; ++i) {
                level.add(new ItemEntity(new ResourceItem(Resource.boxium), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
            }
