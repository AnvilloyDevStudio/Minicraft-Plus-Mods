...
public class PepperTile extends Tile
{
    public PepperTile(final int id) {
        super(id);
    }
    ...
    public void render(final Screen screen, final Level level, final int x, final int y) {
        ...
        int icon = age / 10;
        if (icon >= 3) {
            col = Color.get(level.dirtColor - 121, level.dirtColor - 11, 40 + icon * 100, 40 + (icon - 3) * 2 * 100);
            if (age == 50) {
                col = Color.get(500, 0, 40 + icon * 100, 40 + (icon - 3) * 2 * 100);
            }
            icon = 3;
        }
    ...
    private void harvest(final Level level, final int x, final int y) {
        ...
        for (int count = this.random.nextInt(3), i = 0; i < count; ++i) {
            level.add(new ItemEntity(new ResourceItem(Resource.pepperSeeds), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
        }
        int count = 0;
        if (age == 50) {
        ...
        for (int i = 0; i < count; ++i) {
            level.add(new ItemEntity(new ResourceItem(Resource.pepper), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
        }
