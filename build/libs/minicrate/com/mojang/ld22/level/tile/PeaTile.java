...
public class PeaTile extends Tile
{
    public int isWand;
    public int isStaff;
    
    public PeaTile(final int id) {
        super(id);
    ...
    public void render(final Screen screen, final Level level, final int x, final int y) {
        int col = Color.get(10, 30, 152, level.grassColor);
    ...
    public void tick(final Level level, final int xt, final int yt) {
        if (this.random.nextInt(200) == 0) {
            level.add(new ItemEntity(new ResourceItem(Resource.pea), xt * 16 + this.random.nextInt(6) + 5, yt * 16 + this.random.nextInt(6) + 5));
        }
        ...
        if (level.getTile(xn, yn) == Tile.dirt) {
            level.setTile(xn, yn, PeaTile.pea, 0);
        ...
        if (this.isWand == 0) {
            ...
            if (damage >= 5) {
                for (int count = this.random.nextInt(2) + 1, i = 0; i < count; ++i) {
                    level.add(new ItemEntity(new ResourceItem(Resource.peanut), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
                }
            ...
            level.setTile(x, y, Tile.bean, 0);
            level.setFire(x, y, 0);
            this.isWand = 0;
        }
        if (this.isStaff == 1) {
            level.setTile(x, y, Tile.lettuce, 0);
            level.setFire(x, y, 0);
            this.isStaff = 0;
        }
    }
