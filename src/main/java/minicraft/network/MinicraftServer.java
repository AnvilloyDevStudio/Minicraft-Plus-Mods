...
public class MinicraftServer extends Thread implements MinicraftProtocol {
	...
	public MinicraftServerThread getAssociatedThread(RemotePlayer player) {
		...
		if (thread == null) {
			System.err.println("SERVER: Could not find thread for remote player " + player);
	...
	boolean parsePacket(MinicraftServerThread serverThread, InputType inType, String alldata) {
		...
		switch(inType) {
				...
				if (serverThread.getClient().getIpAddress().isLoopbackAddress() && hostPlayer == null) {
					...
					if (Game.player != null) {
						...
						try {
							...
							System.err.println("SERVER: Server had error while trying to load host player data from file:");
							ex.printStackTrace();
						}
					}
			...
			case LOAD:
				...
				for (Entity curEntity : entities) {
					...
					if (curEntity != clientPlayer) {
						...
						if (Game.debug && curEntity instanceof Player)
							System.out.println("SERVER: Sending player in ENTITIES packet: " + curEntity);
					}
			...
			case ENTITY:
				...
				if (Game.debug) System.out.println("SERVER: Sending entity addition via " + serverThread + " because client requested it: " + entityToSend);
