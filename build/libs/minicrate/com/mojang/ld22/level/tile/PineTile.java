...
public class PineTile extends Tile
{
    public int isWand;
    public int isStaff;
    
    public PineTile(final int id) {
        super(id);
    ...
    public void render(final Screen screen, final Level level, final int x, final int y) {
        ...
        if (u && ul && l) {
            screen.render(x * 16 + 0, y * 16 + 0, 56, col, 0);
        }
        else {
            screen.render(x * 16 + 0, y * 16 + 0, 23, col, 0);
        }
        if (u && ur && r) {
            screen.render(x * 16 + 8, y * 16 + 0, 88, barkCol2, 0);
        }
        else {
            screen.render(x * 16 + 8, y * 16 + 0, 24, col, 0);
        }
        if (d && dl && l) {
            screen.render(x * 16 + 0, y * 16 + 8, 88, barkCol2, 0);
        }
        else {
            screen.render(x * 16 + 0, y * 16 + 8, 55, barkCol1, 0);
        }
        if (d && dr && r) {
            screen.render(x * 16 + 8, y * 16 + 8, 56, col, 0);
        }
        else {
            screen.render(x * 16 + 8, y * 16 + 8, 120, barkCol2, 0);
        }
    ...
    public void tick(final Level level, final int xt, final int yt) {
        ...
        if (level.getTile(xn, yn) == Tile.dirt) {
            level.setTile(xn, yn, PineTile.bPine, 0);
    ...
    public void bumpedInto(final Level level, final int x, final int y, final Entity entity) {
        if (this.random.nextInt(100) == 1 && entity instanceof Player) {
            ...
            level.add(new ItemEntity(new ResourceItem(Resource.herbs), x * 16 + noCenterX + 3, y * 16 + noCenterY + 3));
        }
        if (level.getFire(x, y) > 0) {
    ...
    private void hurt(final Level level, final int x, final int y, final int dmg) {
        if (this.isWand == 0) {
            for (int count = (this.random.nextInt(3) == 0) ? 1 : 0, i = 0; i < count; ++i) {
                level.add(new ItemEntity(new ResourceItem(Resource.pinon), x * 16 + this.random.nextInt(20) + 3, y * 16 + this.random.nextInt(20) + 3));
            }
            ...
            if (damage >= 20) {
                ...
                for (int count2 = this.random.nextInt(4) + 1, j = 0; j < count2; ++j) {
                    level.add(new ItemEntity(new ResourceItem(Resource.pinon), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
                }
                for (int count2 = this.random.nextInt(4) + 1, j = 0; j < count2; ++j) {
                    level.add(new ItemEntity(new ResourceItem(Resource.cone), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
                }
            ...
            level.setTile(x, y, Tile.elm, 0);
            level.setFire(x, y, 0);
            this.isWand = 0;
        }
        if (this.isStaff == 1) {
            level.setTile(x, y, Tile.cloudCactus, 0);
            level.setFire(x, y, 1);
            this.isStaff = 0;
        }
    }
