...
import minicraft.mod.ModItem.ModItemLevel;
import minicraft.mod.ModItem.ToolType;
...
public class ModToolItem extends Item {
    ...
    protected static ArrayList<Item> getAllInstances() {
        ...
        for (ToolType tool : ModItem.ToolTypes) {
            if (!tool.noLevel) {
                for (ModItemLevel lvl : tool.levels)
    ...
    public ModItemLevel level; // Level of said tool
    public int dur; // The durability of the tool
    public boolean attack; // Attackable
    ...
    public ModToolItem(ToolType type, ModItemLevel level) {this(type, level, true);}
    public ModToolItem(ToolType type, ModItemLevel level, boolean attack) {
    ...
    public ModToolItem(ToolType type) {this(type, true);}
    public ModToolItem(ToolType type, boolean attack) {
        super(type.name, new Sprite(type.xPos, type.yPos, 0));
