...
public class Muncher extends Mob
{
    private int xa;
    private int ya;
    private int lvl;
    ...
    public void render(final Screen screen) {
        ...
        int col = Color.get(-1, 100, 310, 444);
        if (this.lvl == 2) {
            col = Color.get(-1, 100, 310, 444);
        }
        if (this.lvl == 3) {
            col = Color.get(-1, 100, 310, 444);
        }
    ...
    protected void die() {
        ...
        for (int count = this.random.nextInt(10) + 1, i = 0; i < count; ++i) {
            this.level.add(new ItemEntity(new ResourceItem(Resource.ironOre), this.x + this.random.nextInt(11) - 5, this.y + this.random.nextInt(11) - 5));
        }
        ...
        for (int count = this.random.nextInt(10) + 1, i = 0; i < count; ++i) {
            this.level.add(new ItemEntity(new ResourceItem(Resource.silverOre), this.x + this.random.nextInt(11) - 5, this.y + this.random.nextInt(11) - 5));
        }
