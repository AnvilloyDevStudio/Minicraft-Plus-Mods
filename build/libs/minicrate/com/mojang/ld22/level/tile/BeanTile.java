...
public class BeanTile extends Tile
{
    public int isWand;
    public int isStaff;
    
    public BeanTile(final int id) {
        super(id);
    ...
    public void render(final Screen screen, final Level level, final int x, final int y) {
        int col = Color.get(10, 30, 251, level.grassColor);
    ...
    public void tick(final Level level, final int xt, final int yt) {
        if (this.random.nextInt(100) == 0) {
            level.add(new ItemEntity(new ResourceItem(Resource.bean), xt * 16 + this.random.nextInt(6) + 5, yt * 16 + this.random.nextInt(6) + 5));
        }
        ...
        if (level.getTile(xn, yn) == Tile.dirt) {
            level.setTile(xn, yn, BeanTile.bean, 0);
