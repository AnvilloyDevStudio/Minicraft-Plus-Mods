...
public class Renderer extends Game {
		...
		if (showinfo) { // Renders show debug info on the screen.
			...
			if (!isValidServer()) {
				...
				if (levels[currentLevel] != null)
					info.add("Tile: " + ((Tile)levels[currentLevel].getTile(player.x >> 4, player.y >> 4)).name);
	...
	static java.awt.Dimension getWindowSize() {
		return new java.awt.Dimension(Float.valueOf(WIDTH * SCALE).intValue(), Float.valueOf(HEIGHT * SCALE).intValue());
	}
}
