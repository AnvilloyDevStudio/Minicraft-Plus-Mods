...
{
    public CarrotTile(final int id) {
        super(id);
    }
    ...
    public void render(final Screen screen, final Level level, final int x, final int y) {
        ...
        if (icon >= 3) {
            ...
            if (age == 50) {
                col = Color.get(511, 0, 50 + icon * 100, 40 + (icon - 3) * 2 * 100);
            }
            icon = 3;
        }
    ...
    private void harvest(final Level level, final int x, final int y) {
        ...
        for (int count = this.random.nextInt(2), i = 0; i < count; ++i) {
            level.add(new ItemEntity(new ResourceItem(Resource.carrotSeed), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
        }
        int count = 0;
        if (age == 50) {
