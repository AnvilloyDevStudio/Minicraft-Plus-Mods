...
public class BElmTile extends Tile
{
    private Tile onType;
    
    public BElmTile(final int id) {
        super(id);
        this.onType = BElmTile.grass;
    ...
    public void tick(final Level level, final int x, final int y) {
        ...
        if (age > 100) {
            level.setTile(x, y, BElmTile.elm, 0);
            level.setFire(x, y, 0);
        }
        else {
    ...
    public void hurt(final Level level, final int x, final int y, final Mob source, final int dmg, final int attackDir) {
        level.setTile(x, y, BElmTile.grass, 0);
        level.setFire(x, y, 0);
    }
}
