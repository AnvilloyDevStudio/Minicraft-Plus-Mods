...
public class CornTile extends Tile
{
    ...
    public void render(final Screen screen, final Level level, final int x, final int y) {
        ...
        if (icon >= 3) {
            ...
            if (age == 50) {
                col = Color.get(533, 0, 50 + icon * 100, 40 + (icon - 3) * 2 * 100);
            }
            icon = 3;
        }
        screen.render(x * 16 + 0, y * 16 + 0, 113, col, 0);
        screen.render(x * 16 + 8, y * 16 + 0, 113, col, 0);
        screen.render(x * 16 + 0, y * 16 + 8, 113, col, 1);
        screen.render(x * 16 + 8, y * 16 + 8, 113, col, 1);
    ...
    private void harvest(final Level level, final int x, final int y) {
        ...
        for (int count = this.random.nextInt(3), i = 0; i < count; ++i) {
            level.add(new ItemEntity(new ResourceItem(Resource.cornSeeds), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
        }
        int count = 0;
        if (age == 50) {
        ...
        for (int i = 0; i < count; ++i) {
            level.add(new ItemEntity(new ResourceItem(Resource.corn), x * 16 + this.random.nextInt(10) + 3, y * 16 + this.random.nextInt(10) + 3));
        }
